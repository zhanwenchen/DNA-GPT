...
def detection(text, option, config):
...
def generate_html(key, option, text, config):
    ...
    res, max_over_lap_generatedtext = detection(text, option, config)
    text = text.lower()
...
def main():
    ...
    input_group = [input_key, radio, input_text, cfg]
